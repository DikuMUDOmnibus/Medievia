...
void show_char_to_char(struct char_data *i, struct char_data *ch, int mode)
{
	...
	/* A char without long descr, or not in default pos. */
	    if (!IS_NPC(i)) {   
			if(i->specials.stpMount&&GET_ZONE(i)!=180){
				sprintf(buffer,"%s riding %s",GET_NAME(i),i->specials.stpMount->player.short_descr);
			}else{
	    ...
	    if (IS_SET(ORIGINAL(i)->specials.act, PLR_LOG)&& IS_PLAYER(ch,"Io"))
		strcat(buffer," (L)");
	...
	if (IS_AFFECTED(i,AFF_SANCTUARY)){
	    global_color=31;
	    act("    $n is surrounded by a shimmering, magical field.", FALSE, i, 0, ch, TO_VICT);
	    global_color=0;
        }
	if (IS_AFFECTED(i,AFF_FIRESHIELD)){
	    global_color=31;
	    act("    $n is enshrouded in a deep red shield of fire.", FALSE, i, 0, ch,TO_VICT);
	    global_color=0;
        }
...
return;
	...
	if (ch == i) continue;
	if(IS_MOB(i)&&i->specials.stpMount)continue;
	if ( ((IS_AFFECTED(ch, AFF_SENSE_LIFE) && !i->specials.wizInvis)
	     || (GET_ZONE(ch)==198)||
		...
		((!IS_SET(EXIT(ch, door)->exit_info, EX_CLOSED)) ||
		 (!IS_SET(EXIT(ch, door)->exit_info, EX_SECRET) &&
		  ...
		  GET_LEVEL(ch) > 33 ){
			...
			if (GET_LEVEL(ch) > 31)
			  sprintf(buf + strlen(buf), " [%d]", world[EXIT(ch, door)->to_room]->number);
			...
			else if (IS_SET(EXIT(ch, door)->exit_info, EX_ILLUSION))
			  sprintf(buf + strlen(buf), " - %s\r\n", "[Illusion]");
			else if (IS_DARK(EXIT(ch, door)->to_room) && !ch->specials.holyLite&&world[ch->in_room]->zone!=198)
			  sprintf(buf + strlen(buf), " - %s\n\r", "Darkness..");
			else
...
"Your prey is in the immediate area with you!\r\n"
	...
	if(!stpCh->specials.hunting) {
		log_hd("### SHOWING TRACKING MESSAGES WHEN NO ONE BEING TRACKED.");
		return;
	}
...
void do_score(struct char_data *ch, char *argument, int cmd)
{
   ...
   if(!IS_NPC(ch)&&ch->p && ch->p->stpFreight&&!IS_FLYING(ch)) {
		if(world[ch->p->stpFreight->iLocationRoom]->zone!=197){
			sprintf(buf,"Freight Location %s:%s Somewhere in %s.\n\r",BLU(ch),NRM(ch),
			zone_table[world[ch->p->stpFreight->iLocationRoom]->zone].name);
			send_to_char(buf,ch);
		}else{
   		sprintf(buf,"Freight Location %s:%s ",BLU(ch),NRM(ch));
  				iX = ch->p->stpFreight->iLocationX;
  				iY = ch->p->stpFreight->iLocationY;
  		iAngle = iHoloWhere(HOLOX(ch), HOLOY(ch), iX, iY, &iDistance);
  		if(iAngle >= 360)
  			iClock = 12;
  		else if(iAngle >= 330)
  			iClock = 11;
  		...
  		send_to_char(buf,ch);
  		if(iAngle == -1)
  			sprintf(buf,"Your freight is right with you.\r\n");
  		else
  			sprintf(buf,"Your freight is at %d o'Clock %s.\r\n",iClock,szaSurveyDists[(int) iMes]);
...
void do_who(struct char_data *ch, char *argument, int cmd)
{
	...
	if ((!strcmp(arg,"-n"))&&name[0]){
	    ...
	    if (!IS_NPC(whoname)){
	    	sprintf(buf,"[TL-%d :: LV-%d] [PKs-%d :: Clan-%d] %s\n\r",
			get_total_level(whoname), GET_LEVEL(whoname),
			whoname->specials.numpkills, whoname->specials.clan,
			GET_NAME(whoname));
		send_to_char(buf,ch);
		return;
	    }else{
...
\r  TL Lv CC  S   PKs   Clan Name    Status......
\r------------------------------------------------------------------------------
\r");
	send_to_char(log_buf,ch);
    global_color=0;
...
/*			ORIGINAL(d->character)->specials.numkills, */
		  ...
		  global_color=33;
		  send_to_char(buf,ch);
		  if ((d->character->specials.clan) && (GET_LEVEL(d->character) < 33))
		    sprintf(buf, "[%2d] ", d->character->specials.clan);
		  else
		    sprintf(buf, "[  ] ");
		  send_to_char(buf,ch);
		  global_color=0;
...
FILE *fp;
	...
	if(!szBuf||!szBuf[0]){
		...
		if(szBuf[0]=='n'&&GET_LEVEL(ch)>=33){
			...
			med_close(fp);
			send_to_char("NEWS NEW NEWS NEW NEWS NEW NEWS!!! :)  \n\r",ch);
			return;
		}
...
*/
	      if(iHoloChDistance(ch,d->character) < 50) {
