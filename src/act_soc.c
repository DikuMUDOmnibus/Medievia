...
int number_socials;
extern void FreightMobsToRoom(int iRoom);
extern void FreightMobsFromRoom(int iRoom);
...
char *fread_action(FILE *fl)
{
	...
	if (feof(fl))
	{
	    log_hd("Fread_action - unexpected EOF.");
	    exit(0);
	}
...
void boot_social_messages(void)
{
    ...
    fclose(fl);
	open_files--;
    number_socials=list_top;
}

void do_socials(struct char_data *ch, char *argument, int comd)
{
int cmd;
    send_to_char("     CURRENT SOCIALS COMMANDS LISTING\n\r",ch);
    for ( cmd = 0; cmd <= list_top; cmd++ ){
		sprintf(log_buf,"%s, ",soc_mess_list[cmd].name);
...
bool check_social( struct char_data *ch, char *pcomm, int length, char *arg )
{
 ...
 LCmdFound:
	...
	*buf = '\0';
	...
	FreightMobsFromRoom(ch->in_room);
    return TRUE;
}
