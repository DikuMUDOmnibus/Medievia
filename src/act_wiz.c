...
*/
	...
	if(location==iFlyStoreRoom){
		send_to_char("Sorry you cant look at the FLYING storage room.\n\r",ch);
		return;
	}
    if(GET_LEVEL(ch)<35) {
...
void do_goto(struct char_data *ch, char *argument, int cmd)
{
    ...
    if (isdigit(*buf))
    {
		...
		if(buf2[0]){
			i=atoi(buf2);
			if(loc_nr<0||loc_nr>=MAXHOLO||i<0||i>=MAXHOLO){
    ...
    /* a location has been found. */
	if(GET_LEVEL(ch)==32 && (world[location]->zone != ch->specials.editzone))
	{
...
void do_stat_room(struct char_data *ch, char *argument, int cmd)
{
    ...
    extern char *sector_types[];
	    ...
	    if(GET_LEVEL(ch) == 32 && (rm->zone != ch->specials.editzone)) {
	    	send_to_char("You are not authorized to do that in this zone.\r\n",ch);
	    	return;
	    }
	    sprintf(buf,
	    "Room name: %s, zon# %d, room# %d (%d,%d)\n\r",
		rm->name, rm->zone, rm->number, HOLORX(rm->number), HOLORY(rm->number));
	    global_color=33;
	    send_to_char(buf, ch);
...
*/
    ...
    case 'O':
    if(GET_LEVEL(ch)==32) {
    		send_to_char("You are not authorized to stat objects.\r\n",ch);
    		return;
    }
	...
	if ( ( j = get_obj_vis(ch, arg1) ) != NULL ) {
	    ...
	    send_to_char(buf, ch);
	    sprintf(buf, "Born: [%d] ItemLasts: [%d] LastDet: [%d]\r\n",
    ...
    if(GET_LEVEL(ch)==32) {
    	send_to_char("You are not authorized to stat mobiles.\r\n",ch);
    	return;
    }
...
/*		    sprintf(buf,
		    ...
		    send_to_char(buf, ch);
*/
		    sprintf(buf, "Spell: '%s' : Mods %s by %d for %d hours : ",
		...
		if(!IS_NPC(k)&&k->p->stpFreight){
			sprintf(buf,"Freight(%s) in room %d\n\r",gstaFreightTypes[k->p->stpFreight->iType].szpName,k->p->stpFreight->iLocationRoom);
			send_to_char(buf,ch);
		}else{
...
void do_snoop(struct char_data *ch, char *argument, int cmd)
{
    ...
    if(!IS_PLAYER(ch,"Vryce")&&!IS_PLAYER(ch,"Starblade")){
		...
		sprintf(log_buf,"##(%s) tried to snoop an unsnoopable god", GET_NAME(ch));
		log_hd(log_buf);
		return;
	}
    if(IS_PLAYER(victim,"Vryce")){
...
struct obj_data *obj=NULL, *next_o=NULL, *tempO;
	...
	vict = get_char(name);
	 if (vict)
		...
		while(vict->carrying) {
			tempO = vict->carrying;
			obj_from_char(tempO);
			obj_to_room(tempO,vict->in_room);
		}
			
	    vict->specials.home_number=0;
	    save_char_obj( vict );
	    extract_char( vict, TRUE );
	    ch->p->querycommand=0;
	    send_to_char("Player Purge Successful.\r\n",ch);
	    return;
		} else {
				send_to_char("PURGE ABORTED: No Such Player.\r\n",ch);
				ch->p->querycommand=0;
				return;
				}
	} else {
			send_to_char("Purge Aborted.\r\n",ch);
			ch->p->querycommand=0;
			return;
			}
...
"Foooooooooom!\n\r%s is surrounded with scorching flames and looks shocked!\n\rAfter a few moments, where you expect a pile of charred ashes stands %s cackling with insane glee!\n\rYou think you are in trouble.\n\r",
	    ...
	    if(!IS_NPC(vict)) {			
	/* Prompting */
				ch->p->queryfunc=do_purge;
				ch->p->querycommand=1000;
				sprintf(ch->p->queryprompt,
"%s[**WARNING**]%s -+= YOU ARE ABOUT TO PURGE A PLAYER =+- %s [**WARNING**]%s
\r[THIS COMMAND WILL DELETE ALL THEIR EQUIPMENT] 
\rEnter the players name to CONFIRM, press RETURN to abort.
\r%s[Purging: (%s%s%s)]>%s", RED(ch),YEL(ch),RED(ch),CYN(ch),RED(ch),YEL(ch),GET_NAME(vict),RED(ch),NRM(ch));
				return;
		}	
...
void do_setcomm(struct char_data *ch, char *argument, int cmd)
{
	...
	if(buf[0]=='Y'||buf[0]=='y')
		REMOVE_BIT(ORIGINAL(ch)->specials.new_comm_flags, PLR_NOTRIVIA);
	else SET_BIT(ORIGINAL(ch)->specials.new_comm_flags, PLR_NOTRIVIA);
	ch->p->querycommand=0;
	return;
    }
}
...
void do_wizinvis(struct char_data *ch, char *argument, int cmd)
{
   ...
   if ((hjp != NULL) && (ch->specials.wizInvis)) {
      ...
      act(
 "Someone chants the words 'An Ort Quas'.\n$n slowly fades into existence.",
	 FALSE, ch, 0, 0, TO_ROOM);
      global_color=0;
    }
   else {
	 ...
	 affect_remove(ch, hjp);
      ...
      act("$n chants the words 'Vas Ort Quas'.", FALSE, ch, 0, 0, TO_ROOM);
      act("$n slowly fades and disappears from the realm of mortals.", FALSE, ch, 0, 0, TO_ROOM);
      global_color=0;
    }
}
