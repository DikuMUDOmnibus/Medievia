...
[11]-SWAMP		wading through a marshing swamp/forest
[12]-JUNGLE		walking through a misty, humid jungle
[13]-ARCTIC		frigidly cold wasteland of snow and ice
[14]-MANA		high mana regeneration room
",ch);

      	strcpy(ch->p->queryprompt,
	 "Select a number 0-14 or CR=no change > ");
	ch->p->querycommand=1000; 
	return; 
    }
    if(cmd==1000&&buf[0]){
	...
	if(number<0||number>14){
	    send_to_char("Select sector type from 0-14 only.\n\r",ch);
	    return;
	}
...
-20=only allow players under level 20 (-35to35(cr to cancel))> "); 
	...
	return;	
 	...
 	strcpy(ch->p->queryprompt,
	 "\n\rEdit the rooms MOUNT/FREIGHT Restrictions? (y/cr)> "); 
	ch->p->querycommand=1013;
	return; 
    }
    if(cmd==1009){
	if(buf[0]=='Y'||buf[0]=='y')
	...
	if(cmd==1013){
		if(buf[0]=='Y'||buf[0]=='y'){
 			strcpy(ch->p->queryprompt,"
1) NO Horse, Mules or Wagons
2) NO Mules and Wagons
3) No Wagons
4) Allow all freight types
Enter in your selection(1,2,3 or 4)> "); 
			ch->p->querycommand=1014;
			return;
		}else{
			ch->p->querycommand=0;
			return;
	...
	if(cmd==1014){
		if(buf[0]=='1'){
			SET_BIT(world[ch->in_room]->extra_flags,TRADE_NO_WAGON);
			SET_BIT(world[ch->in_room]->extra_flags,TRADE_NO_MULE);
			SET_BIT(world[ch->in_room]->extra_flags,TRADE_NO_HORSE);
			ch->p->querycommand=0;
			return;
		}
		if(buf[0]=='2'){
			SET_BIT(world[ch->in_room]->extra_flags,TRADE_NO_WAGON);
			SET_BIT(world[ch->in_room]->extra_flags,TRADE_NO_MULE);
			REMOVE_BIT(world[ch->in_room]->extra_flags,TRADE_NO_HORSE);
			ch->p->querycommand=0;
			return;
		}
		if(buf[0]=='3'){
			SET_BIT(world[ch->in_room]->extra_flags,TRADE_NO_WAGON);
			REMOVE_BIT(world[ch->in_room]->extra_flags,TRADE_NO_MULE);
			REMOVE_BIT(world[ch->in_room]->extra_flags,TRADE_NO_HORSE);
			ch->p->querycommand=0;
			return;
		}
		if(buf[0]=='4'){
			REMOVE_BIT(world[ch->in_room]->extra_flags,TRADE_NO_WAGON);
			REMOVE_BIT(world[ch->in_room]->extra_flags,TRADE_NO_MULE);
			REMOVE_BIT(world[ch->in_room]->extra_flags,TRADE_NO_HORSE);
			ch->p->querycommand=0;
			return;
...
void do_editroom_exit(struct char_data *ch, char *argument, int cmd)
{
   ...
   if(cmd==9){
	...
	/* LEVEL 32's SKIP KEY & DOORNAME /TYPE */
	if(GET_LEVEL(ch)==32) {
	      strcpy(ch->p->queryprompt,
      "
...
Enter ONE line entrance message or ENTER(cr) no change:
 ");
	...
	sprintf(ch->p->queryprompt,"%s\n\rOLD ENTRANCE MESSAGE:\n\r%s\r\n>",
		ch->p->queryprompt, world[ch->in_room]->dir_option[ch->internal_use]->entrance);
      } else sprintf(ch->p->queryprompt,"%s>",ch->p->queryprompt);
...
void do_grouptell(struct char_data *ch, char *argument, int cmd)
{
  ...
  if(ch->desc && !(ch->desc->connected == CON_SOCIAL_ZONE)) {
	send_to_char("You can't use formtell here.\r\n",ch);
	return;
  }
