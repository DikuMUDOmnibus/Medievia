...
if ( check_playing( d, GET_NAME(ch) ) )
	...
	else
	{
	    ...
	    if(!Checkcase(tmp_name)){
 		write_to_q( "Illegal name, Only first letter may be uppercase.\n\rName: ", &d->output );
 		STATE(d)=CON_GET_NAME;
		return;
	    }
	    /* New player */
...
/* gracefully.                                                          */
	...
	if ( ch->in_room < 0 && GET_LEVEL(ch) != 0) {
		log_hd("##### CHAR WITH A *NEGATIVE* START ROOM!");
		ch->in_room = 3099;
	}
	if ( ch->in_room >= 2 )
	    {
...
*/
		...
		if(ch->p->iMount){
			stpMount=read_mobile(ch->p->iMount,REAL);
			if(stpMount)
				char_to_room(stpMount,ch->in_room);
		}
	    }
