...
void cast_wizard_eye( byte level, struct char_data *ch, char *arg, int si, struct char_data *tar_ch, struct obj_data *tar_obj);
void cast_hammer_of_faith( byte level, struct char_data *ch, char *arg, int si, struct char_data *tar_ch, struct obj_data *tar_obj);
void cast_protection_from_good( byte level, struct char_data *ch, char *arg, int si, struct char_data *tar_ch, struct obj_data *tar_obj);
void cast_frost_shards( byte level, struct char_data *ch, char *arg, int si, struct char_data *tar_ch, struct obj_data *tar_obj);
...
void do_cast(struct char_data *ch, char *argument, int cmd)
{
  ...
  if ( ((spl > 0 && spl <= 44) || spl >= 53) && spell_info[spl].spell_pointer )
  {
	...
	if ( ( tar_obj = get_obj_vis(ch, name) ) != NULL )
	  target_ok = TRUE;
	if (GET_LEVEL(ch) == 35) target_ok = TRUE;
      }
	...
	&& (number(1,200) > ch->skills[spl].learned+43+(GET_WIS(ch)-11)*8))
	|| ((GET_CLASS(ch)!=CLASS_CLERIC)
	&& (number(1,200) > ch->skills[spl].learned+43+(GET_INT(ch)-11)*8)) ){
...
void assign_spell_pointers(void)
{
    ...
    SPELLO(90,9,POSITION_STANDING, 17, 32, 15,
     TAR_IGNORE, cast_farsight);

    SPELLO(91,6,POSITION_STANDING, 32, 32, 20,
    ...
    SPELLO(94,18,POSITION_STANDING, 32, 26, 50,
     TAR_IGNORE, cast_bloodbath);

    SPELLO(95,18,POSITION_STANDING, 32, 27, 150,
     TAR_IGNORE, cast_resurrect);

    SPELLO(97, 18, POSITION_STANDING, 20, 32, 200, TAR_IGNORE, 
	cast_wizard_eye);
    ...
    SPELLO( 1,9,POSITION_STANDING, 32,  1, 5,
...
/*  SPELLO( 9,18,POSITION_STANDING,15, 32, 40,
    ...
    SPELLO(12,6,POSITION_STANDING,32,  3, 5,
    ...
    SPELLO(20,9,POSITION_STANDING, 2,  3, 5,
     TAR_CHAR_ROOM | TAR_SELF_ONLY, cast_detect_magic);

    SPELLO(21,6,POSITION_STANDING,32,  2, 5,
    ...
    SPELLO(29,9,POSITION_STANDING, 4, 32, 5,
    ...
    SPELLO(99,18,POSITION_STANDING,32,  6, 5,
    ...
    SPELLO(44,9,POSITION_STANDING,32,  7, 5,
    ...
    SPELLO(56,9,POSITION_STANDING, 7, 12, 10, TAR_CHAR_ROOM,
	   cast_fly);

    SPELLO(86,18,POSITION_STANDING, 10, 12, 10, TAR_CHAR_ROOM,
	   cast_breath_water);

    SPELLO(57,6,POSITION_STANDING, 4, 2, 7, TAR_IGNORE,
	   cast_cont_light);
    ...
    SPELLO(60,36,POSITION_STANDING, 9, 32, 33, TAR_IGNORE,
    ...
    SPELLO(67,9,POSITION_STANDING, 13, 32, 12, TAR_CHAR_ROOM|TAR_SELF_ONLY,
	   cast_shield);
    ...
    SPELLO(71,36,POSITION_STANDING, 26, 19, 250, TAR_IGNORE,
	   cast_gate);
    ...
    SPELLO(77,9,POSITION_STANDING, 6, 9, 5, TAR_CHAR_ROOM,
	   cast_infravision);
